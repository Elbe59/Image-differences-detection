...
def add_original_image(adress):
    global image_ref
    image_ref = ImageTk.PhotoImage(Image.open(adress).resize((480, 320), Image.ANTIALIAS))


def add_results_image(adress,image_name,confusion_matrix,data_result):
    global image_list
    my_img = ImageTk.PhotoImage(Image.open(adress).resize((480, 320), Image.ANTIALIAS))
